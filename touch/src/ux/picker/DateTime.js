Ext.define('Ext.ux.picker.DateTime', {
	...
	doneButton: "OK"  ,
         ...
         * @accessor
         */
        monthText: (Ext.os.deviceType.toLowerCase() == "phone") ? 'M' : 'Mois',
         ...
         * @accessor
         */
        dayText: (Ext.os.deviceType.toLowerCase() == "phone") ? 'j' : 'Jour',
         ...
         * @accessor
         */
        yearText: (Ext.os.deviceType.toLowerCase() == "phone") ? 'A' : 'Année',

        /**
        * @cfg {String} hourText
        * The label to show for the hour column. Defaults to 'Hour'.
        */
        hourText: (Ext.os.deviceType.toLowerCase() == "phone") ? 'H' : 'Heure',
	...
	secondeText: (Ext.os.deviceType.toLowerCase() == "phone") ? 's' : 'Seconde',
        /**
         * @cfg {String} ampmText
         ...
         * @accessor
         */
        slotOrder: ['month', 'day', 'year','hour','minute','seconde','ampm'],
    ...
    getValue: function() {
        var values = {},
            daysInMonth, day, hour, minute, seconde,
        ...
        day = Math.min(values.day, daysInMonth),hour = values.hour,  minute = values.minute,  seconde = values.seconde;
	    ...
	    secondeval = (isNaN(seconde)) ? new Date().getSeconds() : seconde;
        ...
        return new Date(yearval, monthval, dayval, hourval, minuteval, secondeval);
    },
	...
	for(i=0;i<60;i+=this.getSecondeInterval()){
            secondes.push({
                text: this.pad2(i),
                value: i
            });
        ...
        slotOrder.forEach(function(item) {
            slots.push(this.createSlot(item, days, months, years,hours,minutes,secondes,ampm));
        }, this);
     ...
     * @private
     */
    createSlot: function(name, days, months, years,hours,minutes,secondes,ampm) {
        switch (name) {
            case 'year':
                return {
                    name: 'year',
